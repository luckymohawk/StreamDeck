...
# --- IMPORTS for correct path handling ---
...
try:
    ...
    print("Please install necessary packages (e.g., pip install streamdeck Pillow Flask Flask-CORS platformdirs)", file=sys.stderr)
    sys.exit(1)
...
def resource_path(relative_path):
    ...
    try:
        ...
        # Not running in a bundle, so the base path is the script's directory
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path)

# === Application Directories & Files (Corrected for Bundling) ===
# Use platformdirs to find the safe, writable location for user data.
...
# Writable data (like the DB) goes in the user's Application Support folder.
DB_PATH = APP_DATA_DIR / "streamdeck.db"

# NOTE: The following paths are now handled by `resource_path()` and are no
# longer needed as global variables.
# LOAD_SCRIPT, SCRIPTS_DIR, WEB_UI_DIR are now obsolete.
...
# System fonts are safe to use with absolute paths.
...
def load_applescript_template(template_filename, **kwargs):
    # This function is now simplified to use the bundle-aware resource_path.
    # The path is relative to the bundle's root (e.g., "scripts/my_template.applescript")
    try:
        # Construct the path to the resource within the bundle
        filepath_to_use = resource_path(template_filename)
        ...
        raise RuntimeError(f"Could not load template '{template_filename}': {e}")
...
def run_initial_setup_scripts():
    ...
    for script_name in scripts_to_run:
        try:
            # Use resource_path to find the script inside the bundle
            script_path = resource_path(os.path.join("scripts", script_name))
        except FileNotFoundError:
...
def execute_applescript_dialog(prompt_message, default_answer=""):
    # The template name now directly maps to the file in the bundle
    ...
    script = load_applescript_template("scripts/system_events_dialog.applescript", **script_vars)
...
def execute_applescript_confirm(prompt_message):
    ...
    script = load_applescript_template("scripts/system_events_confirm.applescript", **script_vars)
    proc = subprocess.run(["osascript", "-"], input=script, text=True, capture_output=True, check=False)
...
def get_active_terminal_window_name():
    ...
    try:
        script = load_applescript_template("scripts/get_active_terminal_window.applescript")
...
def activate_terminal_window(window_name):
    ...
    try:
        script = load_applescript_template("scripts/activate_terminal_window.applescript", window_name=window_name)
...
def get_terminal_output(window_title):
    ...
    try:
        script = load_applescript_template("scripts/terminal_check_text.applescript", **script_vars)
...
def get_items():
    # Reads from the DB_PATH in the safe user data directory
    try:
...
def log_command_to_file(log_path_str, full_command_str):
    # This now logs to the correct application support directory.
    try:
        log_dir = Path(log_path_str)
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file = log_dir / "streamdeck_commander.log"
...
def log_to_recpath(recpath, message_type, content):
    """Logs a message to log.txt, falling back to APP_DATA_DIR on error."""
    log_dir_to_try = None
    try:
        # First, try the user-provided RECPATH if it exists
        if recpath and recpath != "{{RECPATH}}":
            log_dir_to_try = Path(recpath)
        else:
            # Otherwise, default to the standard app data directory.
            log_dir_to_try = APP_DATA_DIR
            
        log_dir_to_try.mkdir(parents=True, exist_ok=True)
        log_file = log_dir_to_try / "log.txt"
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        with open(log_file, "a", encoding='utf-8') as f:
            f.write(f"[{timestamp}] - {message_type}: {content}\n")

    except Exception as e:
        print(f"[ERROR] CRITICAL: Failed to write to recording log file in '{log_dir_to_try}': {e}")
        print("  Please check permissions for that directory or your RECPATH variable.")
...
def send_keystroke_to_terminal(window_title, keystroke):
    ...
    try:
        script = load_applescript_template("scripts/terminal_keystroke.applescript", **script_vars)
...
def run_cmd_in_terminal(main_cmd, is_at_act=False, at_has_n=False, btn_style_cfg=None, act_at_lbl=None, is_n_staged=False, ssh_staged="", n_staged="", prepend="", force_new_win_at=False, force_local_execution=False, script_template_override=None, ssh_cmd_to_keystroke=None, actual_cmd_to_keystroke=None):
    ...
    # All template paths are now relative to the bundle root
    tpl_map = {"spawn_ssh_and_snapshot": "scripts/terminal_spawn_ssh_and_snapshot.applescript","spawn_and_snapshot": "scripts/terminal_spawn_and_snapshot.applescript","n_staged": "scripts/terminal_n_for_at_staged_keystroke.applescript","at_n": "scripts/terminal_activate_new_styled_at_n.applescript","at_only": "scripts/terminal_activate_found_at_only.applescript","n_alone": "scripts/terminal_activate_standalone_n.applescript","to_active_at": "scripts/terminal_command_to_active_at_device.applescript","default": "scripts/terminal_do_script_default.applescript","force_local_new_window": "scripts/terminal_force_new_window_and_do_script.applescript"}
...
def monitor_window_snapshot(global_idx, window_id, initial_snapshot, keyword, generation_id):
    ...
    while global_idx in monitor_threads and monitor_generations.get(global_idx) == generation_id:
        ...
        try:
            ...
            script = load_applescript_template("scripts/get_window_content.applescript", window_id=window_id)
...
def build_page(idx_param):
    ...
    for i, item in enumerate(items):
        ...
        if is_inplace_sticky: inplace_sticky_items.append((i, item))
        elif is_layout_sticky: layout_sticky_items.append((i, item))
        else: normal_items.append((i, item))
    slots_taken = set()
...
def render_individual_key(i_key):
    ...
    if g_idx is not None and g_idx < len(items):
        item = items[g_idx]; lbl_render, cmd_render, f_str_render = item.get('label',''), item.get('command',''), item.get('flags','')
    ...
    if record_flag:
        state_info=record_toggle_states.get(g_idx,{"state":"OFF"});state=state_info.get("state","OFF");W,H=deck.key_image_format()['size'];img=PILHelper.create_image(deck);draw=ImageDraw.Draw(img);final_bg_hex=bg_color;status_text_to_draw=None
        if state=="ERROR":final_bg_hex=BASE_COLORS['R']if flash_state else dim_color(BASE_COLORS['R']);status_text_to_draw="ERROR"
        final_text_color=text_color(final_bg_hex)
        try:pil_bg=tuple(int(final_bg_hex.lstrip('#')[i:i+2],16)for i in(0,2,4))
        except:pil_bg=(0,0,0)
        draw.rectangle([(0,0),(W,H)],fill=pil_bg)
        if state=="RECORDING"and flash_state:ellipse_fill=tuple(int(BASE_COLORS['R'].lstrip('#')[i:i+2],16)for i in(0,2,4));draw.ellipse([(10,10),(W-10,H-10)],fill=ellipse_fill);final_text_color=text_color(BASE_COLORS['R'])
        try:font_label=ImageFont.truetype(FONT_PATH,fs);font_take=ImageFont.truetype(BOLD_FONT_PATH,16);font_status=ImageFont.truetype(FONT_PATH,11)
        except IOError:font_label,font_take,font_status=ImageFont.load_default(),ImageFont.load_default(),ImageFont.load_default()
        label_y_pos=H*0.45
        if status_text_to_draw:label_y_pos=H*0.55;s_bbox=font_status.getbbox(status_text_to_draw,anchor="lt")if hasattr(font_status,'getbbox')else(0,0,*draw.textsize(status_text_to_draw,font=font_status));draw.text(((W-(s_bbox[2]-s_bbox[0]))/2,5),status_text_to_draw,font=font_status,fill=final_text_color)
        wrapped_label="\n".join(textwrap.wrap(lbl_render,width=10,max_lines=2,placeholder="…"));draw.text((W/2,label_y_pos),wrapped_label,font=font_label,fill=final_text_color,anchor="ma",spacing=LINE_SPACING,align="center")
        take_val_str=current_session_vars.get("TAKE","1")
        try:take_val_display=str(int(take_val_str)).zfill(3)
        except(ValueError,TypeError):take_val_display=take_val_str[:3]
        draw.text((W/2,H*0.80),f"TAKE {take_val_display}",font=font_take,fill=final_text_color,anchor="ma");deck.set_key_image(i_key,PILHelper.to_native_format(deck,img));return
    if osa_mon_flag:
        mon_state = monitor_states.get(g_idx)
        if mon_state == "OSA_MONITORING": status_render, should_flash_status_text, bg_render = "MONITOR...", True, bg_color
        elif mon_state == "OSA_FOUND": status_render, bg_render = "FOUND", bg_color if flash_state else dim_color(bg_color)
        elif mon_state == "OSA_GONE": status_render, bg_render = "WIN GONE", BASE_COLORS['R']
        elif mon_state == "OSA_ERROR": status_render, bg_render = "OSA ERROR", BASE_COLORS['R']
        else: status_render, bg_render = "OSA Ready", dim_color(bg_color)
        ...
        if proc.poll()is None:bg_render=BASE_COLORS['B']if flash_state else dim_color(BASE_COLORS['B']);status_render="RUNNING...";txt_override_render=text_color(bg_render)
        else:del background_processes[g_idx]
    if 'V' in f_str_render:
        vars_to_display=[str(current_session_vars[match.group(1).strip()])for match in VAR_PATTERN.finditer(cmd_render)if match.group(1).strip()in current_session_vars]
    ...
    if numeric_mode and long_press_numeric_active:
        ...
        if i_key==num_key or i_key in[up_key_idx,down_key_idx]:
            _,_,_,num_orig_bg,_,_,_,_,_,_,_,_=parse_flags(flags.get(num_key,""));bright_num_bg=toggle_button_bg(num_orig_bg);bg_render=bright_num_bg if flash_state else(num_orig_bg if i_key==num_key else dim_color(bright_num_bg));txt_override_render=text_color(bg_render)
    ...
    final_fs = 22 if i_key == load_key_idx else (ARROW_FONT_SIZE if i_key in [up_key_idx, down_key_idx] else fs)
    try: deck.set_key_image(i_key, render_key(lbl_render, deck, bg_render, final_fs, txt_override_render, status_render, vars_render, flash_active=(should_flash_status_text and flash_state), extra_text=extra_txt))
    except Exception as e: print(f"[ERROR] Render key {i_key} failed: {e}", file=sys.stderr); traceback.print_exc()

def start_monitoring():
    global items, monitor_threads, monitor_states, current_session_vars, monitor_generations
    for g_idx in list(monitor_threads.keys()): monitor_generations[g_idx] = None; monitor_threads.pop(g_idx, None)
    for g_idx, item_data in enumerate(items):
        ...
        if monitor_flag and '@' in item_flags_mon:
            monitor_states[g_idx] = 'initializing'; current_gen_id = time.time(); monitor_generations[g_idx] = current_gen_id
...
def load_data_and_reinit_vars():
    ...
    try:
        ...
        # Use resource_path to find the bundled database creation script
        load_script_path = resource_path("streamdeck_db.py")
        # The script will create the database in the safe APP_DATA_DIR
...
def callback(deck_param, k_idx, pressed):
    ...
    if g_idx_cb is not None and g_idx_cb < len(items):
        item_data = items[g_idx_cb]; orig_item_cmd_from_db = item_data.get('command',''); lbl_str = item_data.get('label',''); flag_str = item_data.get('flags','')
    if numeric_mode and long_press_numeric_active:
        num_key = numeric_var['key']
        if k_idx == num_key: numeric_mode, numeric_var, long_press_numeric_active = False, None, False; toggle_keys.clear(); build_page(page_index); return
            ...
            new = curr + step if k_idx == up_key_idx else curr - new
            current_session_vars[numeric_var['name']] = new; cmd_run = resolve_command_string(numeric_var['cmd_template'], current_session_vars)
            if numeric_var.get('is_background'): subprocess.Popen(shlex.split(cmd_run))
            else: run_cmd_in_terminal(cmd_run, act_at_lbl=labels.get(active_device_key), force_local_execution=numeric_var.get('force_local', False))
            build_page(page_index); return
        else: numeric_mode, numeric_var, long_press_numeric_active = False, None, False; toggle_keys.clear(); build_page(page_index); return
    if g_idx_cb is None:
        if k_idx==down_key_idx and lp:
            web_ui_path = resource_path("browsebuttons") # Path to bundled React app
            if 'web_ui_process' in globals() and (web_ui_process is None or web_ui_process.poll() is not None):
                if not Path(web_ui_path).exists() or not Path(web_ui_path, "package.json").exists(): return
                try: web_ui_process=subprocess.Popen(['npm','run','dev'],cwd=web_ui_path,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True); time.sleep(5)
    ...
    if background_flag:
        if g_idx_cb in background_processes and background_processes[g_idx_cb].poll() is None:
            ...
            try: background_processes[g_idx_cb].wait(timeout=2)
            except subprocess.TimeoutExpired: background_processes[g_idx_cb].kill()
            ...
            try:
                ...
                if active_device_key is not None and not force_local_cb:
                    ...
                    if active_at_is_mobile: active_at_cmd = _transform_ssh_user_for_mobile(active_at_cmd)
                    ssh_match = re.match(r"^(ssh\s+\S+)", active_at_cmd)
                    if ssh_match:
                        ssh_base = ssh_match.group(1); escaped_res_cmd = res_cmd.replace('"', '\\"'); final_bg_cmd = f'{ssh_base} "{escaped_res_cmd}"'
                        print(f"[INFO] Executing remote background command: {final_bg_cmd}")
                    else: print(f"[ERROR] Background task failed: Active @ device is not a valid SSH command."); return
                else: final_bg_cmd = res_cmd; print(f"[INFO] Executing local background command: {final_bg_cmd}")
                proc = subprocess.Popen(shlex.split(final_bg_cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE); background_processes[g_idx_cb] = proc
            except Exception as e: print(f"[ERROR] Failed to run background command: {e}", file=sys.stderr)
        build_page(page_index); return
    if record_flag:
        recpath = resolve_command_string("{{RECPATH}}", current_session_vars)
        if not recpath or recpath == "{{RECPATH}}": recpath = None
        state_info = record_toggle_states.get(g_idx_cb, {"state": "OFF"}); current_state = state_info.get("state", "OFF")
        if lp:
            ...
            for match in variables_to_edit:
                var_name = match.group(1).strip(); default_val = match.group(3) if match.group(3) is not None else ""; current_val = str(current_session_vars.get(var_name, default_val))
                user_input = execute_applescript_dialog(f"Enter value for {var_name}:", current_val)
                if user_input is None or user_input in ["USER_CANCELLED_PROMPT", "USER_TIMEOUT_PROMPT"]: build_page(page_index); return
                if user_input != current_val: current_session_vars[var_name] = user_input
            new_scene_value = str(current_session_vars.get('SCENE', '')); scene_changed = original_scene_value != new_scene_value
            take_match = re.search(r'\{\{TAKE:([^}]+)\}\}', orig_item_cmd_from_db, re.IGNORECASE); default_take_str = "1"
            if take_match and take_match.group(1).isdigit(): default_take_str = take_match.group(1)
            suggested_take = default_take_str if scene_changed else str(current_session_vars.get('TAKE', default_take_str)); prompt_message_take = f"SCENE changed. Reset TAKE or enter new value:" if scene_changed else "Enter TAKE number:"
            user_input_take = execute_applescript_dialog(prompt_message_take, suggested_take)
            if user_input_take and user_input_take not in ["USER_CANCELLED_PROMPT", "USER_TIMEOUT_PROMPT"]: current_session_vars['TAKE'] = user_input_take
            build_page(page_index); return
        if current_state == "ERROR": record_toggle_states.pop(g_idx_cb, None); build_page(page_index); return
        if current_state == "OFF":
            if active_device_key is None: print("[ERROR] REC Start: No active @-device selected."); record_toggle_states[g_idx_cb] = {"state": "ERROR"}; build_page(page_index); return
            active_at_cmd = resolve_command_string(cmds.get(active_device_key, ""), current_session_vars)
            _, _, _, _, _, _, active_at_is_mobile, _, _, _, _, _ = parse_flags(flags.get(active_device_key, "")); final_command = resolve_command_string(orig_item_cmd_from_db, current_session_vars); target_window_title = ""
            if active_at_is_mobile: target_window_title = labels.get(active_device_key); run_cmd_in_terminal(final_command, act_at_lbl=target_window_title)
            else:
                target_window_title = f"{lbl_str}-REC"; print(f"[INFO] * button '{lbl_str}' targeting non-mobile @-device. Spawning new mobile session for recording.")
                mobile_ssh_cmd = _transform_ssh_user_for_mobile(active_at_cmd); btn_cfg_for_new_win = {"lbl": target_window_title, "bg_hex": bg_cb, "text_color_name": text_color(bg_cb)}
                run_cmd_in_terminal("", is_n_staged=True, ssh_staged=mobile_ssh_cmd, n_staged=final_command, btn_style_cfg=btn_cfg_for_new_win)
            time.sleep(0.5)
            if recpath:
                terminal_output = get_terminal_output(target_window_title); log_content = final_command; prefix = "CMD"
                if terminal_output:
                    match = re.search(r"(Will capture.*?Session start status: 0)", terminal_output, re.DOTALL)
                    if match: log_content = match.group(1).strip(); prefix = "START_OUTPUT"
                log_to_recpath(recpath, prefix, log_content)
            record_toggle_states[g_idx_cb] = {"state": "RECORDING", "window_title": target_window_title}; build_page(page_index); return
        elif current_state == "RECORDING":
            target_window_title = state_info.get("window_title")
            if not target_window_title: print("[ERROR] REC Stop: Cannot find window title from recording start."); record_toggle_states[g_idx_cb] = {"state": "ERROR"}; build_page(page_index); return
            send_keystroke_to_terminal(target_window_title, "\\r"); time.sleep(0.5); terminal_output = get_terminal_output(target_window_title); has_error = False
            if terminal_output:
                ...
                for line in terminal_output.split('\n')[-15:]:
                    if any(keyword.lower() in line.lower() for keyword in error_keywords):
                        has_error = True; print(f"[INFO] Recording error detected in window '{target_window_title}'.")
                        if recpath: log_to_recpath(recpath, "ERR", line.strip())
                        break
            if has_error: record_toggle_states[g_idx_cb] = {"state": "ERROR"}
            else:
                try: current_take_num = int(current_session_vars.get('TAKE', "1")); current_session_vars['TAKE'] = str(current_take_num + 1)
                except (ValueError, TypeError): current_session_vars['TAKE'] = "1"
                record_toggle_states.pop(g_idx_cb, None)
            build_page(page_index); return
        return
    if osa_mon_flag and not lp:
        current_mon_state = monitor_states.get(g_idx_cb)
        if current_mon_state in ["OSA_MONITORING", "OSA_FOUND"]: print(f"[INFO] User cancelled/dismissed OSA monitor for button {g_idx_cb}."); monitor_generations[g_idx_cb] = None; monitor_states.pop(g_idx_cb, None); redraw(); return
        if g_idx_cb in monitor_threads and monitor_generations.get(g_idx_cb) is not None: monitor_generations[g_idx_cb] = None; time.sleep(0.1)
        keyword=item_data.get('monitor_keyword',''); keyword = keyword[:-2] if keyword.endswith(".0") else keyword
        if not keyword: monitor_states[g_idx_cb]='OSA_ERROR'; print("[ERROR] OSA Monitor keyword is missing."); redraw(); return
        command_to_run=resolve_command_string(orig_item_cmd_from_db,current_session_vars); style={"lbl":lbl_str,"bg_hex":bg_cb,"text_color_name":text_color(bg_cb)}; result_str=None
        if active_device_key is not None:
            ssh_cmd=resolve_command_string(cmds.get(active_device_key,""),current_session_vars)
            if ssh_cmd: result_str=run_cmd_in_terminal("",btn_style_cfg=style,script_template_override="spawn_ssh_and_snapshot",ssh_cmd_to_keystroke=ssh_cmd,actual_cmd_to_keystroke=command_to_run)
        else: result_str=run_cmd_in_terminal(command_to_run,btn_style_cfg=style,script_template_override="spawn_and_snapshot")
        if result_str and "::::" in result_str:
            ...
            if window_id_str.isdigit():
                ...
                thread=threading.Thread(target=monitor_window_snapshot,args=(g_idx_cb,window_id,initial_snapshot,keyword,gen_id),daemon=True); monitor_threads[g_idx_cb]=thread;thread.start()
            else: monitor_states[g_idx_cb]='OSA_ERROR'
        else: monitor_states[g_idx_cb]='OSA_ERROR'
        redraw(); return
    if '#' in flag_str:
        if lp:
        ...
        else: run_cmd_in_terminal(res_cmd, act_at_lbl=labels.get(active_device_key), force_local_execution=force_local_cb)
        ...
        if dev_cb: at_devices_to_reinit_cmd.add(k_idx)
...
if __name__ == "__main__":
    ...
    print(f"[INFO] User data directory is: {APP_DATA_DIR}")

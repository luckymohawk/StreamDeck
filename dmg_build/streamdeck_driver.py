...
# This is the full, corrected callback function.
# Replacing the entire function guarantees correct indentation.
def callback(deck_param, k_idx, pressed):
    ...
    duration = time.time() - press_times.pop(k_idx, time.time())
    lp = duration >= LONG_PRESS_THRESHOLD
    ...
    # --- THIS IS THE CORRECTED BLOCK FOR THE DOWN ARROW LONG-PRESS ---
    # It is now correctly indented within the function.
    if k_idx == down_key_idx and lp:
        # Check if the web UI process needs to be started
        # The python script will inherit the PATH set by the .command script, so it can find 'npm'
        if 'web_ui_process' not in globals() or (web_ui_process and web_ui_process.poll() is not None):
            if not WEB_UI_DIR.exists() or not (WEB_UI_DIR / "package.json").exists():
                return

            try:
                command_to_run = ['npm', 'run', 'dev']
                print(f"[INFO] Launching Web UI with command: {' '.join(command_to_run)}")
                ...
                print(f"[ERROR] Failed to start Web UI server from Python: {e}", file=sys.stderr)
                return
        
        # This now runs regardless of whether the process was just started or already running
        webbrowser.open(f"http://localhost:{REACT_APP_DEV_PORT}")
        return

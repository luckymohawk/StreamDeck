-- Final Combined Script (Version 16)
-- 1. Removes "K" as a color flag. It is now treated only as a functional flag.
-- 2. All previous functionality and bug fixes are retained.
	...
	tell document 1
		tell sheet "Streamdeck"
			tell table 1
				...
				-- =================================================================
				-- Part 2: Update Colors (With "K" flag removed as a color)
				-- =================================================================
				try
					...
					repeat with i from 2 to rowCount
						...
						if cell_value is missing value then
							...
							-- Step 1: Determine the base color and text color
							set base_bg_color to missing value
							set base_text_color to {0, 0, 0}
							set color_is_set to false
							
							-- THE FIX: "K" is no longer checked for here.
							if flag contains "R" then
								set base_bg_color to {65535, 0, 0}
								set base_text_color to {65535, 65535, 65535}
								set color_is_set to true
							else if flag contains "G" then
								set base_bg_color to {0, 65535, 0}
								set color_is_set to true
							else if flag contains "B" then
								set base_bg_color to {0, 26214, 52428}
								set base_text_color to {65535, 65535, 65535}
								set color_is_set to true
							else if flag contains "O" then
								set base_bg_color to {65535, 39321, 0}
								set color_is_set to true
							else if flag contains "Y" then
								set base_bg_color to {65535, 65535, 0}
								set color_is_set to true
							else if flag contains "P" then
								set base_bg_color to {32896, 0, 32896}
								set base_text_color to {65535, 65535, 65535}
								set color_is_set to true
							else if flag contains "S" then
								set base_bg_color to {49344, 49344, 49344}
								set color_is_set to true
							else if flag contains "F" then
								set base_bg_color to {65535, 0, 65535}
								set color_is_set to true
							else if flag contains "W" then
								set base_bg_color to {65535, 65535, 65535}
								set color_is_set to true
							else if flag contains "L" then
								set base_bg_color to {65021, 63222, 58365}
								set color_is_set to true
							end if
							
							-- Step 2: If a color was found, check for the "D" modifier
							if color_is_set and (flag contains "D") then
								set r to item 1 of base_bg_color
								set g to item 2 of base_bg_color
								set b to item 3 of base_bg_color
								
								-- Dim the color by 50%
								set base_bg_color to {(r * 0.5) as integer, (g * 0.5) as integer, (b * 0.5) as integer}
							end if
							
							-- Step 3: Apply the final calculated colors
							tell cell ("A" & i)
								set background color to base_bg_color
								set text color to base_text_color
							end tell
						end if
					end repeat
				end try
				...
				-- =================================================================
				-- Part 3 & 4: Variable Formatting and Fallback (Unchanged)
				-- =================================================================
				...
				repeat with i from 2 to lastRow
					...
					try
						...
						if cellD_value contains "{{" and cellD_value contains "}}" then
							set shouldProcessRowDue to false
						end if
					on error
					end try
